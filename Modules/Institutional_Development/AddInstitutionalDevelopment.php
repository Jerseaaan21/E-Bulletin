...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['pdfFile']) && $_FILES['pdfFile']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($fileTmpPath, $destPath)) {
            ...
            // Insert record into the database with status set to 'Pending'
            $query = "INSERT INTO department_post (module, description, file_path, content, status, dept_id, user_id) VALUES (?, ?, ?, ?, 'Pending', ?, ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("isssii", $moduleId, $description, $newFilename, $contentData, $dept_id, $user_id);
