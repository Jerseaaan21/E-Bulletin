<?php
// Modules/Institutional_Development/InstitutionalDevelopmentBulletin.php
include "../../db.php";
session_start();
...
// Get APPROVED institutional development documents from department_post table filtered by category
$genderDevelopment = [];
$studentDevelopment = [];
$strategicPlan = [];

// Get all APPROVED Institutional Development documents from department_post
$query = "SELECT * FROM department_post WHERE status = 'approved' AND module = ? AND dept_id = ? ORDER BY created_at DESC";
$stmt = $conn->prepare($query);
$stmt->bind_param("ii", $InstDev_Id, $dept_id);
$stmt->execute();
...
<body class="bg-gray-50 h-screen w-screen overflow-hidden m-0 p-0">
                            ...
                            <?php if ($displayCategory === 'strategic'): ?>
                                ...
                                <div class="h-full w-full bg-white">
                                    <div class="instdev-pdf-preview" id="instdev-preview-<?= $index ?>" 
                                         ...
                                         onclick="openInstDevPDFInParent('<?= $doc['file_path'] ?>', '<?= htmlspecialchars($doc['description']) ?>', '<?= date('F j, Y', strtotime($doc['posted_on'])) ?>', '<?= basename($doc['file_path']) ?>', '<?= $doc['file_type'] ?>')"
    ...
    <script>
        ...
        function openInstDevPDFInParent(filePath, title, postedDate, fileName, fileType) {
