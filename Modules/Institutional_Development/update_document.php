...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] === UPLOAD_ERR_OK) {
        ...
        $query = "SELECT file_path, content FROM department_post WHERE id = ?";
        ...
        // Get the new filename (already generated in JavaScript)
        ...
        if (move_uploaded_file($fileTmpPath, $destPath)) {
            // Preserve category information in content
            $contentData = json_decode($row['content'], true) ?? [];
            $newContent = json_encode($contentData);
            
            // Update the record in the database
            $query = "UPDATE department_post SET description = ?, file_path = ?, content = ?, status = 'Pending' WHERE id = ?";
            $stmt = $conn->prepare($query);
        ...
        // No new file, just update the description but preserve category
        // Get current content to preserve category
        $query = "SELECT content FROM department_post WHERE id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("i", $id);
        $stmt->execute();
        $result = $stmt->get_result();
        ...
        $query = "UPDATE department_post SET description = ?, content = ?, status = 'Pending' WHERE id = ?";
        $stmt = $conn->prepare($query);
