...
{
    $stmt = $conn->prepare("SELECT * FROM CEIT_Organization WHERE position_code = ? AND department_id = 2");
...
{
    $stmt = $conn->prepare("SELECT * FROM CEIT_Organization WHERE position_code LIKE ? AND department_id = 2 ORDER BY 
                            CASE 
...
{
    $stmt = $conn->prepare("SELECT * FROM CEIT_Organization WHERE position_code LIKE 'coordinator_%' AND department_id = 2 ORDER BY id ASC");
    $stmt->execute();
    $result = $stmt->get_result();
    $coordinators = [];
    while ($row = $result->fetch_assoc()) {
...
<body>
<div class="container w-[1150px] mx-auto px-4 py-6 text-center org-container" id="orgChartContainer">
            ...
            <div class="org-chart border-2 border-dit-dark rounded-xl p-6 bg-dit-medium/15 shadow-lg space-y-8 ">
                ...
                <div class="relative group">
                    <p class="text-center font-bold bg-dit-medium text-white rounded-lg text-2xl mb-0 p-2" id="coordinatorsLabel">
                        <?php echo htmlspecialchars(getOrganizationLabel('label1', 2, $conn)); ?>
                    </p>
                    <button 
                ...
                <div class="relative group">
                    <p class="text-center font-bold bg-dit-medium text-white rounded-lg text-2xl mb-0 p-2" id="facultyLabel">
                        <?php echo htmlspecialchars(getOrganizationLabel('label2', 2, $conn)); ?>
                    </p>
                    <button 
                        class="absolute right-2 top-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 text-white bg-dit-medium hover:text-dit-dark hover:bg-white bg-opacity-70 px-2 py-1 rounded-md"
...
<div id="memberModal" class="fixed inset-0 hidden z-50 bg-black bg-opacity-50 flex items-center justify-center">
    <div class="bg-white p-6 rounded-lg w-full max-w-md shadow-2xl">
        <form id="memberForm" method="post" enctype="multipart/form-data">
            ...
            <input type="hidden" name="department_id" id="department_id" value="2">
            <div class="mb-3">
...
<script>
    ...
    function attachEventListeners() {
        ...
        document.querySelectorAll('.add-coordinator-btn').forEach(button => {
            button.removeEventListener('click', handleAddCoordinatorClick);
        });
        ...
        document.querySelectorAll('.add-coordinator-btn').forEach(button => {
            button.addEventListener('click', handleAddCoordinatorClick);
        });
    ...
    function handleAddCoordinatorClick() {
    ...
    function handleLabelFormSubmit(e) {
        ...
        fetch('../OrganizationalChart/CEIT_Label_process.php', {
            method: 'POST',
            body: formData
        })
        .then(response => response.json())
