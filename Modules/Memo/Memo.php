...
$uploadBaseDir = "../../uploads/{$dept_acronym}/Memo/";
...
while ($row = $result->fetch_assoc()) {
    ...
    $relativeFilePath = "uploads/{$dept_acronym}/Memo/" . $row['file_path'];
...
<head>
    ...
    <title>Department Memo Management</title>
    ...
    <style>
        ...
        .memo-status-section.pending {
            background-color: #fef3c7;
            border: 1px solid #fcd34d;
        }

        .memo-status-section.approved {
            background-color: #d1fae5;
            border: 1px solid #6ee7b7;
        }

        .memo-status-section.not-approved {
            background-color: #fee2e2;
            border: 1px solid #fca5a5;
        }

        .memo-status-section.archived {
        ...
        .memo-status-section.pending .memo-status-title {
            color: #d97706;
            border-color: #d97706;
        }

        .memo-status-section.approved .memo-status-title {
            color: #059669;
            border-color: #059669;
        }

        .memo-status-section.not-approved .memo-status-title {
            color: #dc2626;
            border-color: #dc2626;
        }

        .memo-status-section.archived .memo-status-title {
            color: #2563eb;
            border-color: #2563eb;
        }
        ...
        .memo-notification {
            ...
            display: flex;
            align-items: center;
            animation: memo-slideIn 0.3s ease-out;
        }
        ...
        .memo-rejection-tooltip {
            position: relative;
            display: inline-block;
        }

        .memo-rejection-tooltip .memo-tooltip-text {
            visibility: hidden;
            width: 250px;
        ...
        .memo-rejection-tooltip:hover .memo-tooltip-text {
            visibility: visible;
            opacity: 1;
        }
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row justify-between items-center mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-blue-600 mb-4 md:mb-0">
                <i class="fas fa-file-alt mr-3 w-5"></i> Department Memo Management
            </h1>
        ...
        <!-- Pending Memos -->
        <div class="memo-status-section pending">
            <h2 class="memo-status-title">Pending Memos</h2>
            <?php if (count($pendingMemos) > 0): ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($pendingMemos as $index => $memo): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-yellow-500 transition duration-200 transform hover:scale-105">
                            <div class="mb-3 border border-gray-300 rounded">
                                <div id="memo-file-preview-pending-<?= $index ?>" class="memo-file-preview" data-file-path="<?= $memo['file_path'] ?>">
                                    ...
                                    <?php else: ?>
                                        <i class="fas fa-file memo-file-icon <?= $memo['file_type'] ?> text-4xl"></i>
                                    <?php endif; ?>
                                </div>
                            </div>
                            ...
                            <div class="flex justify-end mt-4 space-x-2 text-xs">
                                <button id="memo-view-full-pending-<?= $index ?>" class="p-2 border rounded-lg border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white transition duration-200 transform hover:scale-110" title="View Full Document" data-file-type="<?= $memo['file_type'] ?>" data-file-path="<?= $memo['file_path'] ?>">
                                    <i class="fas fa-eye fa-sm"></i>
                                </button>
                                <button class="p-2 border border-blue-500 text-blue-500 rounded-lg hover:bg-blue-500 hover:text-white transition duration-200 transform hover:scale-110 memo-download-btn" data-file-path="<?= $memo['file_path'] ?>" data-file-name="<?= basename($memo['file_path']) ?>" title="Download">
                                    <i class="fas fa-download fa-sm"></i>
                                </button>
                                <button class="p-2 border border-green-500 text-green-500 rounded-lg hover:bg-green-500 hover:text-white transition duration-200 transform hover:scale-110 memo-edit-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" data-file-path="<?= $memo['file_path'] ?>" data-status="pending" title="Edit">
                                    <i class="fas fa-edit fa-sm"></i>
                                </button>
                                <button class="p-2 border border-red-500 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition duration-200 transform hover:scale-110 memo-delete-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" data-status="pending" title="Delete">
        ...
        <!-- Approved Memos -->
        <div class="memo-status-section approved">
            <h2 class="memo-status-title">Approved Memos</h2>
            <?php if (count($approvedMemos) > 0): ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($approvedMemos as $index => $memo): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-green-500 transition duration-200 transform hover:scale-105">
                            <div class="mb-3 border border-gray-300 rounded">
                                <div id="memo-file-preview-approved-<?= $index ?>" class="memo-file-preview" data-file-path="<?= $memo['file_path'] ?>">
                                    <?php if ($memo['file_type'] === 'pdf'): ?>
                                        <!-- Loading spinner will be replaced by PDF preview -->
                                        <div class="memo-loading-spinner"></div>
                                    <?php elseif (in_array($memo['file_type'], ['jpg', 'jpeg', 'png', 'gif'])): ?>
                            ...
                            <div class="flex justify-end mt-4 space-x-2 text-xs">
                                <button id="memo-view-full-approved-<?= $index ?>" class="p-2 border rounded-lg border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white transition duration-200 transform hover:scale-110" title="View Full Document" data-file-type="<?= $memo['file_type'] ?>" data-file-path="<?= $memo['file_path'] ?>">
                                    <i class="fas fa-eye fa-sm"></i>
                                </button>
                                <button class="p-2 border border-blue-500 text-blue-500 rounded-lg hover:bg-blue-500 hover:text-white transition duration-200 transform hover:scale-110 memo-download-btn" data-file-path="<?= $memo['file_path'] ?>" data-file-name="<?= basename($memo['file_path']) ?>" title="Download">
                                    <i class="fas fa-download fa-sm"></i>
                                </button>
                                <button class="p-2 border border-yellow-500 text-yellow-500 rounded-lg hover:bg-yellow-500 hover:text-white transition duration-200 transform hover:scale-110 memo-archive-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" data-status="approved" title="Archive">
            ...
            <?php else: ?>
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-lg">No approved memos</p>
                </div>
            <?php endif; ?>
        </div>

        <!-- Not Approved Memos -->
        <div class="memo-status-section not-approved">
            <h2 class="memo-status-title">Not Approved Memos</h2>
            <?php if (count($notApprovedMemos) > 0): ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($notApprovedMemos as $index => $memo): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-red-500 transition duration-200 transform hover:scale-105 opacity-75">
                            <div class="mb-3 border border-gray-300 rounded">
                                <div id="memo-file-preview-not-approved-<?= $index ?>" class="memo-file-preview" data-file-path="<?= $memo['file_path'] ?>">
                                    <?php if ($memo['file_type'] === 'pdf'): ?>
                                        <!-- Loading spinner will be replaced by PDF preview -->
                                        <div class="memo-loading-spinner"></div>
                                    <?php elseif (in_array($memo['file_type'], ['jpg', 'jpeg', 'png', 'gif'])): ?>
                            ...
                            <div class="card-body flex-grow">
                                ...
                                <?php if (!empty($memo['rejection_reason'])): ?>
                                    <div class="memo-rejection-tooltip mt-2">
                                        <button class="text-xs text-red-500 hover:text-red-700 flex items-center">
                                            <i class="fas fa-info-circle mr-1"></i> Rejection Reason
                                        </button>
                                        <span class="memo-tooltip-text"><?= htmlspecialchars($memo['rejection_reason']) ?></span>
                                    </div>
                                <?php endif; ?>
                            </div>
                            <div class="flex justify-end mt-4 space-x-2 text-xs">
                                <button id="memo-view-full-not-approved-<?= $index ?>" class="p-2 border rounded-lg border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white transition duration-200 transform hover:scale-110" title="View Full Document" data-file-type="<?= $memo['file_type'] ?>" data-file-path="<?= $memo['file_path'] ?>">
                                    <i class="fas fa-eye fa-sm"></i>
                                </button>
                                <button class="p-2 border border-blue-500 text-blue-500 rounded-lg hover:bg-blue-500 hover:text-white transition duration-200 transform hover:scale-110 memo-download-btn" data-file-path="<?= $memo['file_path'] ?>" data-file-name="<?= basename($memo['file_path']) ?>" title="Download">
                                    <i class="fas fa-download fa-sm"></i>
                                </button>
                                <button class="p-2 border border-green-500 text-green-500 rounded-lg hover:bg-green-500 hover:text-white transition duration-200 transform hover:scale-110 memo-edit-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" data-file-path="<?= $memo['file_path'] ?>" data-status="not-approved" title="Edit & Resubmit">
                                    <i class="fas fa-edit fa-sm"></i>
                                </button>
                                <button class="p-2 border border-red-500 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition duration-200 transform hover:scale-110 memo-delete-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" data-status="not-approved" title="Delete">
                                    <i class="fas fa-trash fa-sm"></i>
                                </button>
                            </div>
                        </div>
            ...
            <?php else: ?>
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-lg">No rejected memos</p>
                </div>
            <?php endif; ?>
        </div>

        <!-- Archived Memos -->
        <div class="memo-status-section archived">
            <h2 class="memo-status-title">Archived Memos</h2>
            <?php if (count($archivedMemos) > 0): ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($archivedMemos as $index => $memo): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-blue-500 transition duration-200 transform hover:scale-105 opacity-75">
                            <div class="mb-3 border border-gray-300 rounded">
                                <div id="memo-file-preview-archived-<?= $index ?>" class="memo-file-preview" data-file-path="<?= $memo['file_path'] ?>">
                                    <?php if ($memo['file_type'] === 'pdf'): ?>
                                        <!-- Loading spinner will be replaced by PDF preview -->
                                        <div class="memo-loading-spinner"></div>
                                    <?php elseif (in_array($memo['file_type'], ['jpg', 'jpeg', 'png', 'gif'])): ?>
                            ...
                            <div class="flex justify-end mt-4 space-x-2 text-xs">
                                <button id="memo-view-full-archived-<?= $index ?>" class="p-2 border rounded-lg border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white transition duration-200 transform hover:scale-110" title="View Full Document" data-file-type="<?= $memo['file_type'] ?>" data-file-path="<?= $memo['file_path'] ?>">
                                    <i class="fas fa-eye fa-sm"></i>
                                </button>
                                <button class="p-2 border border-blue-500 text-blue-500 rounded-lg hover:bg-blue-500 hover:text-white transition duration-200 transform hover:scale-110 memo-download-btn" data-file-path="<?= $memo['file_path'] ?>" data-file-name="<?= basename($memo['file_path']) ?>" title="Download">
                                    <i class="fas fa-download fa-sm"></i>
                                </button>
                                <button class="p-2 border border-green-500 text-green-500 rounded-lg hover:bg-green-500 hover:text-white transition duration-200 transform hover:scale-110 memo-restore-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" title="Restore">
                                    <i class="fas fa-undo fa-sm"></i>
                                </button>
                                <button class="p-2 border border-red-500 text-red-500 rounded-lg hover:bg-red-500 hover:text-white transition duration-200 transform hover:scale-110 memo-delete-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" data-status="archived" title="Delete">
                                    <i class="fas fa-trash fa-sm"></i>
                                </button>
                            </div>
                        </div>
        ...
        <div id="memo-file-modal-pending-<?= $index ?>" class="memo-file-modal">
            <div class="memo-modal-content">
                <div class="memo-modal-header">
                    <h3 class="memo-modal-title"><?= htmlspecialchars($memo['description']) ?></h3>
    ...
    <div id="memo-upload-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <form id="memo-upload-form" action="Modules/Memo/AddMemo.php" method="POST" enctype="multipart/form-data" class="space-y-4">
                <div>
    ...
    <div id="memo-edit-modal" class="hidden fixed inset-0 bg-black bg-opacity-70 overflow-y-auto h-full w-full z-50">
        <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div class="mt-3 text-center">
                <h3 class="text-lg leading-6 font-medium text-gray-900" id="memo-edit-modal-title">Edit Memo</h3>
                <form id="memo-edit-form" class="mt-2 py-3" enctype="multipart/form-data">
                    <input type="hidden" id="memo-edit-id" name="id">
                    <input type="hidden" id="memo-edit-status" name="status">
                    <div class="mb-4">
    ...
    <!-- Archive/Delete/Restore Confirmation Modal for Memos -->
    <div id="memo-delete-modal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center hidden z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            <div class="mb-4">
                <h3 class="text-xl font-bold text-yellow-600" id="memo-modal-title">Archive Memo</h3>
            </div>
            <div class="mb-4">
                <p>Are you sure you want to <span id="memo-action-text">archive</span> this memo?</p>
                <p class="font-semibold mt-2" id="memo-delete-title"></p>
            </div>
            <div class="flex justify-end space-x-3">
                <button class="px-4 py-2 border border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white rounded-lg transition duration-200" id="memo-cancel-delete-btn">
                    Cancel
                </button>
                <button class="px-4 py-2 border border-yellow-500 text-yellow-500 hover:bg-yellow-500 hover:text-white rounded-lg transition duration-200" id="memo-archive-btn">
                    <i class="fas fa-archive mr-2"></i> Archive
                </button>
                <button class="px-4 py-2 border border-green-500 text-green-500 hover:bg-green-500 hover:text-white rounded-lg transition duration-200" id="memo-restore-btn" style="display: none;">
                    <i class="fas fa-undo mr-2"></i> Restore
                </button>
                <button class="px-4 py-2 border border-red-500 text-red-500 hover:bg-red-500 hover:text-white rounded-lg transition duration-200" id="memo-confirm-delete-btn">
    ...
    <script>
        ...
        // Global variables for PDF handling - memo specific
        window.memoPdfDocs = {};
        window.memoCurrentPageNum = {};
        window.memoTotalPages = {};
        window.memoIsRendering = {};

        // Global variable to track active requests - memo specific
        ...
        function initializeMemosModule() {
            ...
            if (window.memosModuleInitialized) {
                console.log('Memos module already initialized, reinitializing...');
        ...
        function initializeMemoModalEventListeners() {
            ...
            document.getElementById('memo-upload-btn').addEventListener('click', function() {
                ...
                document.getElementById('memo-upload-modal').classList.remove('hidden');
            });
            ...
            document.getElementById('memo-cancel-upload-btn').addEventListener('click', function() {
                ...
                document.getElementById('memo-upload-modal').classList.add('hidden');
                document.getElementById('memo-upload-form').reset();
            });
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    const status = document.getElementById('memo-edit-status').value;
            ...
            document.querySelectorAll('.memo-edit-btn').forEach(button => {
                ...
                newButton.addEventListener('click', function(e) {
                    ...
                    document.getElementById('memo-edit-status').value = status;
                    document.getElementById('memo-edit-file').value = '';

                    // Update modal title based on status
                    const modalTitle = document.getElementById('memo-edit-modal-title');
                    if (status === 'not-approved') {
                        modalTitle.textContent = 'Edit & Resubmit Memo';
                    } else {
                        modalTitle.textContent = 'Edit Memo';
                    }
            ...
            document.querySelectorAll('.memo-archive-btn, .memo-delete-btn').forEach(button => {
                ...
                newButton.addEventListener('click', function(e) {
                    ...
                    const restoreBtn = document.getElementById('memo-restore-btn');
                    const modalTitle = document.getElementById('memo-modal-title');
                    const actionText = document.getElementById('memo-action-text');

                    if (status === 'approved') {
                        archiveBtn.style.display = 'inline-block';
                        restoreBtn.style.display = 'none';
                        modalTitle.textContent = 'Archive Memo';
                        actionText.textContent = 'archive';
                    } else {
                        archiveBtn.style.display = 'none';
                        restoreBtn.style.display = 'none';
                        modalTitle.textContent = 'Delete Memo';
            ...
            // Restore buttons
            document.querySelectorAll('.memo-restore-btn').forEach(button => {
                // Remove any existing event listeners
                const newButton = button.cloneNode(true);
                button.parentNode.replaceChild(newButton, button);
                ...
                newButton.addEventListener('click', function(e) {
                    ...
                    modalTitle.textContent = 'Restore Memo';
                ...
                newArchiveBtn.addEventListener('click', function(e) {
                    ...
                    if (window.memoToAction) {
                        ...
                        fetch('Modules/Memo/archive_memo.php', {
                                ...
                                body: 'id=' + encodeURIComponent(window.memoToAction)
                            })
                            .then(response => {
            ...
            if (restoreBtn) {
                ...
                newRestoreBtn.addEventListener('click', function(e) {
                    ...
                    if (window.memoToAction) {
                        // Set active request flag
                        memoActiveRequests.restore = true;
                        ...
                        fetch('Modules/Memo/restore_memo.php', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/x-www-form-urlencoded',
                                },
                            ...
                            .then(data => {
                                ...
                                if (data.success) {
                                    // Show success notification
                                    showMemoNotification(data.message || 'Memo restored successfully!', 'success');
                                    ...
                                    // Show error notification
                                    showMemoNotification(data.message || 'Error restoring memo', 'error');
                            ...
                            .catch(error => {
                                ...
                                showMemoNotification('An error occurred while restoring the memo: ' + error.message, 'error');
                            ...
                            .finally(() => {
                                // Reset active request flag
                                memoActiveRequests.restore = false;
                            });
                    }
                });
            }
            ...
            if (deleteBtn) {
                ...
                newDeleteBtn.addEventListener('click', function(e) {
                    ...
                    if (window.memoToAction) {
                        ...
                        console.log("Attempting to delete memo with ID:", window.memoToAction);
        ...
        function handleMemoViewButtonClick(event) {
            ...
            // The id format is "memo-view-full-{status}-{index}"
        ...
        function initializeMemoPDFPreviews() {
            ...
            if (!('IntersectionObserver' in window)) {
                ...
                renderAllMemoPreviews();
            }

            function renderAllMemoPreviews() {
                <?php
                $totalDelay = 0;
                foreach ($pendingMemos as $index => $memo): ?>
                    <?php if ($memo['file_type'] === 'pdf'): ?>
                        setTimeout(() => {
                            renderMemoPDFPreview("<?= $memo['file_path'] ?>", "memo-file-preview-pending-<?= $index ?>");
                        }, <?= $totalDelay + ($index * 200) ?>);
                    <?php endif; ?>
                <?php
                    $totalDelay += count($pendingMemos) * 200;
                endforeach;
                ?>
                ...
                <?php foreach ($approvedMemos as $index => $memo): ?>
                    <?php if ($memo['file_type'] === 'pdf'): ?>
                        setTimeout(() => {
                            renderMemoPDFPreview("<?= $memo['file_path'] ?>", "memo-file-preview-approved-<?= $index ?>");
                        }, <?= $totalDelay + ($index * 200) ?>);
                    <?php endif; ?>
                <?php
                    $totalDelay += count($approvedMemos) * 200;
                endforeach;
                ?>
                ...
                <?php foreach ($notApprovedMemos as $index => $memo): ?>
                    <?php if ($memo['file_type'] === 'pdf'): ?>
                        setTimeout(() => {
                            renderMemoPDFPreview("<?= $memo['file_path'] ?>", "memo-file-preview-not-approved-<?= $index ?>");
                        }, <?= $totalDelay + ($index * 200) ?>);
                    <?php endif; ?>
                <?php
                    $totalDelay += count($notApprovedMemos) * 200;
                endforeach;
                ?>
                ...
                <?php foreach ($archivedMemos as $index => $memo): ?>
                    <?php if ($memo['file_type'] === 'pdf'): ?>
                        setTimeout(() => {
                            renderMemoPDFPreview("<?= $memo['file_path'] ?>", "memo-file-preview-archived-<?= $index ?>");
                        }, <?= $totalDelay + ($index * 200) ?>);
                    <?php endif; ?>
                <?php endforeach; ?>
            }
