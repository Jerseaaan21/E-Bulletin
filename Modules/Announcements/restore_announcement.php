<?php
// Modules/Announcements/restore_announcement.php
session_start();
include "../../db.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Get the announcement details before restoring
    ...
    if ($result->num_rows > 0) {
        ...
        if (strpos($row['file_path'], 'archive/') === 0) {
            ...
            $sourceFile = "../../uploads/{$dept_acronym}/Announcement/archive/" . $fileName;
            $destFile = "../../uploads/{$dept_acronym}/Announcement/" . $fileName;
            ...
            if (file_exists($sourceFile)) {
                ...
                if ($stmt->execute()) {
                    echo json_encode(['success' => true, 'message' => 'Announcement restored successfully to Approved section']);
                } else {
                    http_response_code(500);
                    echo json_encode(['success' => false, 'message' => 'Error restoring announcement: ' . $conn->error]);
                }
            } else {
                ...
                if ($stmt->execute()) {
                    echo json_encode(['success' => true, 'message' => 'Announcement restored successfully to Approved section (file not found in archive)']);
                } else {
                    http_response_code(500);
                    echo json_encode(['success' => false, 'message' => 'Error restoring announcement: ' . $conn->error]);
                }
        ...
        echo json_encode(['success' => false, 'message' => 'Announcement not found or not archived']);
    }
} else {
