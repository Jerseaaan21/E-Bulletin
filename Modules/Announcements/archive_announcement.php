<?php
// Modules/Announcements/archive_announcement.php
session_start();
include "../../db.php";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $query = "SELECT * FROM department_post WHERE id = ? AND dept_id = ? AND status = 'Approved'";
    $stmt = $conn->prepare($query);
    ...
    if ($result->num_rows > 0) {
        ...
        $query = "UPDATE department_post SET status = 'Archived', file_path = CONCAT('archive/', file_path) WHERE id = ? AND dept_id = ?";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("ii", $id, $deptId);
        ...
        echo json_encode(['success' => false, 'message' => 'Announcement not found or not approved']);
    }
} else {
