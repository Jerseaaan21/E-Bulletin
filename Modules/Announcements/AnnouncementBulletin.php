...
// Get approved announcements from the logged-in user's department (excluding CEIT)
$announcements = [];
if ($dept_id != 1) { // Only show if not CEIT
    $query = "SELECT dp.*, d.acronym 
              FROM department_post dp 
              JOIN departments d ON dp.dept_id = d.dept_id 
              WHERE dp.status = 'Approved' AND dp.module = ? AND dp.dept_id = ? 
              ORDER BY dp.created_at DESC";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ii", $Ann_Id, $dept_id);
    $stmt->execute();
    $result = $stmt->get_result();

    while ($row = $result->fetch_assoc()) {
        $fileExtension = pathinfo($row['file_path'], PATHINFO_EXTENSION);
        $relativeFilePath = "uploads/{$row['acronym']}/Announcement/" . $row['file_path'];
