...
// Get module ID for Institutional Development
...
// Get the absolute path for uploads - IMPORTANT FIX HERE
$baseUploadPath = $_SERVER['DOCUMENT_ROOT'] . "/Testing/uploads/";

// Create department directory if it doesn't exist
$deptDir = $baseUploadPath . $dept_acronym . "/";
if (!file_exists($deptDir)) {
    if (!mkdir($deptDir, 0755, true)) {
        header('Content-Type: application/json');
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to create department directory: ' . $deptDir]);
        exit;
    }
}

// Create Institutional Development directory
$uploadDir = $deptDir . "Institutional_Development/";
if (!file_exists($uploadDir)) {
    if (!mkdir($uploadDir, 0755, true)) {
        header('Content-Type: application/json');
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to create Institutional Development directory: ' . $uploadDir]);
        exit;
    }
}
...
if (!file_exists($archiveDir)) {
    if (!mkdir($archiveDir, 0755, true)) {
        header('Content-Type: application/json');
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Failed to create archive directory: ' . $archiveDir]);
        exit;
    }
}
...
error_log("Department Directory: " . $deptDir);
error_log("Upload Directory: " . $uploadDir);
error_log("Archive Directory: " . $archiveDir);

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $description = $_POST['description'];
    $category = $_POST['category'] ?? ''; // Default to empty string if not provided
    ...
    if (isset($_FILES['pdfFile']) && $_FILES['pdfFile']['error'] === UPLOAD_ERR_OK) {
        ...
        // Format: document_DD_MM_YYYY_HH_MM_SS_RANDOM.extension
        $newFilename = "document_{$day}_{$month}_{$year}_{$hours}_{$minutes}_{$seconds}_{$random}.{$fileExtension}";

        // Move file to uploads directory (active documents)
        $destPath = $uploadDir . $newFilename;

        if (move_uploaded_file($fileTmpPath, $destPath)) {
            // Create content JSON with category
            $contentData = json_encode(['category' => $category]);
            
            // Insert record into main_post table with status set to 'active'
            // Note: We store just the filename, not the full path
            $query = "INSERT INTO main_post (module, description, file_path, content, status, user_id) VALUES (?, ?, ?, ?, 'active', ?)";
            $stmt = $conn->prepare($query);
            $stmt->bind_param("isssi", $moduleId, $description, $newFilename, $contentData, $user_id);
            ...
            if ($stmt->execute()) {
                ...
                echo json_encode([
                    ...
                    'file_path' => "uploads/{$dept_acronym}/Institutional_Development/{$newFilename}"
                ]);
                exit;
            } else {
            ...
            header('Content-Type: application/json');
            http_response_code(500);
            echo json_encode(['success' => false, 'message' => 'Error uploading file: ' . ($error['message'] ?? 'Unknown error')]);
            exit;
        }
    } else {
        ...
        if (isset($_FILES['pdfFile']['error'])) {
            switch ($_FILES['pdfFile']['error']) {
                case UPLOAD_ERR_INI_SIZE:
                    $error_message = "Error: The uploaded file exceeds upload_max_filesize directive in php.ini";
                    break;
