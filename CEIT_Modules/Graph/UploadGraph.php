...
function readXLSXFile($filePath) {
    ...
    // Verify file is actually a ZIP file (XLSX files are ZIP archives)
    ...
    if ($fileHandle) {
        ...
        if (substr($header, 0, 2) !== 'PK') {
            throw new Exception('File does not appear to be a valid Excel (.xlsx) file. Please check the file format or try saving as CSV.');
        }
    }
    ...
    try {
        ...
        if ($result !== TRUE) {
            $errorMsg = 'Unable to open Excel file. ';
            switch ($result) {
                case ZipArchive::ER_NOZIP:
                    $errorMsg .= 'File is not a valid ZIP archive (corrupted Excel file).';
                    break;
                case ZipArchive::ER_INCONS:
                    $errorMsg .= 'ZIP archive is inconsistent.';
                    break;
                case ZipArchive::ER_CRC:
                    $errorMsg .= 'CRC error in ZIP archive.';
                    break;
        ...
        // Read styles to detect percentage formatting
        $stylesPath = $tempDir . '/xl/styles.xml';
        $percentageStyles = [];
        if (file_exists($stylesPath)) {
            $stylesXml = simplexml_load_file($stylesPath);
            if ($stylesXml) {
                // Look for percentage number formats
                if (isset($stylesXml->numFmts->numFmt)) {
                    foreach ($stylesXml->numFmts->numFmt as $numFmt) {
                        $formatCode = (string)$numFmt['formatCode'];
                        $numFmtId = (int)$numFmt['numFmtId'];
                        
                        // Check if format contains percentage symbol
                        if (strpos($formatCode, '%') !== false) {
                            $percentageStyles[$numFmtId] = true;
                        }
                    }
                }
                
                // Built-in percentage formats (Excel format IDs 9, 10, 11, 12 are percentages)
                $builtInPercentageFormats = [9, 10, 11, 12];
                foreach ($builtInPercentageFormats as $formatId) {
                    $percentageStyles[$formatId] = true;
                }
                
                // Map cell formats to number formats
                $cellFormats = [];
                if (isset($stylesXml->cellXfs->xf)) {
                    $xfIndex = 0;
                    foreach ($stylesXml->cellXfs->xf as $xf) {
                        $numFmtId = isset($xf['numFmtId']) ? (int)$xf['numFmtId'] : 0;
                        $cellFormats[$xfIndex] = $numFmtId;
                        $xfIndex++;
                    }
                }
            }
        ...
        foreach ($worksheetXml->sheetData->row as $row) {
            ...
            foreach ($row->c as $cell) {
                ...
                if (isset($matches[1])) {
                    ...
                    // Check if this cell is formatted as percentage
                    $isPercentageFormatted = false;
                    if (isset($cellFormats[$styleIndex])) {
                        $numFmtId = $cellFormats[$styleIndex];
                        $isPercentageFormatted = isset($percentageStyles[$numFmtId]);
                    }
                    ...
                    if (isset($cell->v)) {
                        ...
                        if (isset($cell['t']) && (string)$cell['t'] === 's') {
                            ...
                            // If this cell is formatted as percentage and is numeric, convert to percentage display
                            if ($isPercentageFormatted && is_numeric($cellValue)) {
                                $percentageValue = floatval($cellValue) * 100;
            ...
            if (!empty(array_filter($rowData))) {
                if ($rowIndex === 0) {
                    $headers = $rowData;
                    error_log("Excel Headers detected: " . json_encode($headers));
                    error_log("Number of Excel headers: " . count($headers));
                } else {
...
function detectPercentageData($data, $headers) {
            ...
            if (isset($row[$columnName])) {
                ...
                if (strpos($value, '%') !== false) {
                    ...
                    if (is_numeric($numericValue)) {
                        $floatValue = floatval($numericValue);
                        
                        // If value is between 0 and 1, it might be a decimal percentage (0.85 = 85%)
                        // If value is between 0 and 100 and has decimals, it might be a percentage
                        if (($floatValue > 0 && $floatValue <= 1 && $floatValue != intval($floatValue)) ||
                            ($floatValue > 1 && $floatValue <= 100 && strpos($value, '.') !== false)) {
                            // Additional check: if most values in this pattern, likely percentages
                            $totalNumericValues++;
                        } else {
                            $totalNumericValues++;
                        }
    ...
    if ($totalNumericValues > 0 && ($percentageCount / $totalNumericValues) > 0.5) {
        return true;
    }
    
    // Additional check: if values look like percentages (many values between 0-100 with decimals)
    $likelyPercentageCount = 0;
    for ($colIndex = 1; $colIndex < count($headers); $colIndex++) {
        $columnName = $headers[$colIndex];
        ...
        foreach ($data as $row) {
            if (isset($row[$columnName])) {
                ...
                // Remove % if present and check numeric value
                $cleanValue = str_replace(['%', ','], '', $value);
                if (is_numeric($cleanValue)) {
                    $floatValue = floatval($cleanValue);
                    
                    // Check if it looks like a percentage (0-100 range with decimals, or 0-1 range)
                    if (($floatValue >= 0 && $floatValue <= 100 && (strpos($value, '.') !== false || strpos($value, '%') !== false)) ||
                        ($floatValue > 0 && $floatValue <= 1 && $floatValue != intval($floatValue))) {
                        $likelyPercentageCount++;
                    }
                }
            }
        }
    ...
    // If more than 70% of numeric values look like percentages, consider it percentage data
    return $totalNumericValues > 0 && ($likelyPercentageCount / $totalNumericValues) > 0.7;
}
...
function readCSVFileEnhanced($filePath) {
    ...
    if (($handle = fopen($filePath, "r")) !== FALSE) {
        ...
        error_log("CSV Headers detected: " . json_encode($headers));
        error_log("Number of headers: " . count($headers));
        foreach ($headers as $index => $header) {
            error_log("Header $index: '$header'");
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        error_log("Processing file: " . $fileName . " (Extension: " . $fileExtension . ", Size: " . $fileSize . " bytes)");
        error_log("Temporary file path: " . $fileTmpName);
        error_log("Original file name: " . $fileName);
        error_log("Detected extension: " . $fileExtension);
        ...
        if ($fileExtension === 'csv') {
            ...
            try {
                ...
                throw new Exception('Excel file could not be processed. Please try one of these solutions: 1) Save your Excel file as .xlsx format (not .xls), 2) Convert your Excel file to CSV format, 3) Check if your Excel file is password protected or corrupted. Error details: ' . $e->getMessage());
            }
        }
        
        if (empty($fileData['data'])) {
            throw new Exception('No data found in the CSV file or file format is invalid.');
        }
        
        // Validate that we have at least 2 columns
        if (count($fileData['headers']) < 2) {
            throw new Exception('CSV file must contain at least 2 columns (labels and values).');
        }
        
        // Validate that we have at least 2 rows of data
        if (count($fileData['data']) < 2) {
            throw new Exception('CSV file must contain at least 2 rows of data.');
        }
        ...
        // Debug logging
        error_log("Processed data - Headers: " . json_encode($fileData['headers']));
        error_log("Processed data - Column count: " . count($fileData['headers']));
        error_log("Processed data - Row count: " . count($fileData['data']));
        error_log("Processed data - First row: " . json_encode($fileData['data'][0] ?? []));
        error_log("Processed data - Is percentage: " . ($isPercentageData ? 'true' : 'false'));
