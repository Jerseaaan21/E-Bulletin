<?php
// CEIT_Modules/Graphs/archive_graph.php
error_reporting(E_ALL);
ini_set('display_errors', 1);

// Log the request for debugging
error_log("archive_graph.php accessed - Method: " . $_SERVER['REQUEST_METHOD']);
...
if (!$conn) {
    error_log("Database connection failed in archive_graph.php");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    error_log("Attempting to archive graph ID: $id for department: $deptId");
    ...
    $query = "UPDATE main_graph SET status = 'archived' WHERE id = ? AND dept_id = ?";
    $stmt = $conn->prepare($query);
    
    if (!$stmt) {
        error_log("Failed to prepare archive query: " . $conn->error);
        header('Content-Type: application/json');
        http_response_code(500);
        echo json_encode(['success' => false, 'message' => 'Database error: ' . $conn->error]);
        exit;
    ...
    if ($stmt->execute()) {
        if ($stmt->affected_rows > 0) {
            error_log("Graph archived successfully - ID: $id");
            // Return success response
            header('Content-Type: application/json');
            echo json_encode(['success' => true, 'message' => 'Graph archived successfully']);
        } else {
            error_log("No rows affected during archive - ID: $id, Dept: $deptId");
            header('Content-Type: application/json');
            http_response_code(404);
            echo json_encode(['success' => false, 'message' => 'Graph not found or already archived']);
        }
        exit;
    } else {
        error_log("Archive query execution failed: " . $stmt->error);
        header('Content-Type: application/json');
        http_response_code(500);
