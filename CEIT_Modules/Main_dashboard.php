...
<body class="bg-gray-50 min-h-screen font-sans">
    ...
    <script>
        ...
        async function loadModuleContent(moduleId, moduleName) {
            ...
            try {
                // Convert module name to a safe format for URL (keep underscores for file paths)
                const safeModuleName = moduleName.toLowerCase().replace(/\s+/g, '_');
                const response = await fetch(`CEIT_Modules/${safeModuleName}/${safeModuleName}.php`);
        ...
        function cleanupTabResources(tabId) {
            ...
            if (tabId.startsWith('upload-module-')) {
                ...
                const module = mainData?.modules?.find(m => m.id == moduleId);
