...
// Get active memos
$query = "SELECT * FROM main_post WHERE user_id = ? AND status = 'active' AND module = 2 ORDER BY created_at DESC";
$stmt = $conn->prepare($query);
...
// Get archived memos
$query = "SELECT * FROM main_post WHERE user_id = ? AND status = 'archived' AND module = 2 ORDER BY created_at DESC";
$stmt = $conn->prepare($query);
$stmt->bind_param("i", $userId);
$stmt->execute();
$result = $stmt->get_result();
...
<head>
    ...
    <style>
        ...
        .memo-status-section.active .memo-status-title {
            color: #3b82f6;
            border-color: #3b82f6;
        }
...
<body class="bg-gray-50">
    <div class="container mx-auto px-4 py-8">
        <div class="flex flex-col md:flex-row justify-between items-center mb-8">
            <h1 class="text-2xl md:text-3xl font-bold text-blue-600 mb-4 md:mb-0">
                <i class="fas fa-file-alt mr-3 w-5"></i> CEIT Memo Management
            </h1>
        ...
        <!-- Active Memos -->
        <div class="memo-status-section active">
            <h2 class="memo-status-title">Active Memos</h2>
            <?php if (count($activeMemos) > 0): ?>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    <?php foreach ($activeMemos as $index => $memo): ?>
                        <div class="bg-white shadow-md rounded-lg p-4 w-full h-full flex flex-col justify-between border border-blue-500 transition duration-200 transform hover:scale-105">
                            <div class="mb-3 border border-gray-300 rounded">
                                <div id="memo-file-preview-active-<?= $index ?>" class="memo-file-preview" data-file-path="<?= $memo['file_path'] ?>">
                            ...
                            <div class="flex justify-end mt-4 space-x-2 text-xs">
                                <button id="memo-view-full-active-<?= $index ?>" class="p-2 border rounded-lg border-gray-500 text-gray-500 hover:bg-gray-500 hover:text-white transition duration-200 transform hover:scale-110" title="View Full Document" data-file-type="<?= $memo['file_type'] ?>" data-file-path="<?= $memo['file_path'] ?>">
                                ...
                                <button class="p-2 border border-yellow-500 text-yellow-500 rounded-lg hover:bg-yellow-500 hover:text-white transition duration-200 transform hover:scale-110 memo-archive-btn" data-index="<?= $index ?>" data-id="<?= $memo['id'] ?>" data-description="<?= htmlspecialchars($memo['description']) ?>" data-status="active" title="Archive">
            ...
            <?php else: ?>
                <div class="text-center py-8 text-gray-500">
                    ...
                    <p class="text-lg">No active memos yet</p>
                </div>
            <?php endif; ?>
        </div>
    ...
    <!-- File View Modals -->
    <?php foreach ($activeMemos as $index => $memo): ?>
        <div id="memo-file-modal-active-<?= $index ?>" class="memo-file-modal">
            <div class="memo-modal-content">
                <div class="memo-modal-header">
                    <h3 class="memo-modal-title"><?= htmlspecialchars($memo['description']) ?></h3>
                    <span class="memo-modal-close" onclick="closeMemoFileModal('active', <?= $index ?>)">&times;</span>
                </div>
                <div class="memo-modal-body">
                    <div id="memo-pdf-container-active-<?= $index ?>" class="memo-pdf-container">
                        <div class="memo-loading-spinner"></div>
                ...
                <div class="memo-modal-footer">
                    <div class="memo-modal-meta">
                        Posted on: <?= date('F j, Y', strtotime($memo['posted_on'])) ?> | File: <?= basename($memo['file_path']) ?>
                    </div>
                    <?php if ($memo['file_type'] === 'pdf'): ?>
                        <div class="memo-page-navigation">
                            <button id="memo-prev-page-btn-active-<?= $index ?>" class="memo-page-nav-btn" disabled>
                                <i class="fas fa-chevron-left"></i>
                            </button>
                            <div id="memo-page-indicator-active-<?= $index ?>" class="memo-page-indicator">Page 1 of 1</div>
                            <button id="memo-next-page-btn-active-<?= $index ?>" class="memo-page-nav-btn" disabled>
    ...
    <script>
        ...
        function initializeMemoModalEventListeners() {
            ...
            if (uploadForm) {
                uploadForm.addEventListener('submit', function(e) {
                    ...
                    fetch('CEIT_Modules/Memo/AddMemo.php', {
                            method: 'POST',
                            body: formData
                        })
                        .then(response => {
            ...
            if (editForm) {
                editForm.addEventListener('submit', function(e) {
                    ...
                    fetch('CEIT_Modules/Memo/update_memo.php', {
                            method: 'POST',
                            body: formData
                        })
                        .then(response => {
            ...
            if (archiveBtn) {
                ...
                newArchiveBtn.addEventListener('click', function(e) {
                    ...
                    if (window.memoToAction) {
                        ...
                        fetch('CEIT_Modules/Memo/archive_memo.php', {
                                method: 'POST',
                                headers: {
            ...
            if (restoreBtn) {
                ...
                newRestoreBtn.addEventListener('click', function(e) {
                    ...
                    if (window.memoToAction) {
                        ...
                        fetch('CEIT_Modules/Memo/restore_memo.php', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/x-www-form-urlencoded',
                                },
            ...
            if (deleteBtn) {
                ...
                newDeleteBtn.addEventListener('click', function(e) {
                    ...
                    if (window.memoToAction) {
                        ...
                        fetch('CEIT_Modules/Memo/delete_memo.php', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/x-www-form-urlencoded',
                                },
        ...
        function initializeMemoPDFPreviews() {
            ...
            function renderAllMemoPreviews() {
                <?php
                $totalDelay = 0;
                foreach ($activeMemos as $index => $memo): ?>
                    <?php if ($memo['file_type'] === 'pdf'): ?>
                        setTimeout(() => {
                            renderMemoPDFPreview("<?= $memo['file_path'] ?>", "memo-file-preview-active-<?= $index ?>");
                        }, <?= $totalDelay + ($index * 200) ?>);
                    <?php endif; ?>
                <?php
                    $totalDelay += count($activeMemos) * 200;
                endforeach;
                ?>
