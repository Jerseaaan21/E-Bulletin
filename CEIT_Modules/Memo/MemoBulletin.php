<?php
// CEIT_Modules/Memo/MemoBulletin.php
include "../../db.php";
session_start();
...
// Get Memos module ID
$query = "SELECT id FROM Modules WHERE name = 'Memos'";
$result = $conn->query($query);
$Memo_Id = 2; // Default to 2 if not found
...
// Get active memos from main_post table filtered by user_id
$memos = [];
if ($user_id) {
...
<head>
    ...
    <title>CEIT Memos Bulletin</title>
                        ...
                        <div class="carousel-card h-full <?= $index === 0 ? 'active' : '' ?>" data-index="<?= $index ?>">
                            <div class="h-full flex flex-col bg-white" style="max-height: 100vh;">
                                <div class="pdf-preview" id="preview-<?= $index ?>" 
                                     ...
                                     onclick="openPDFInParent('<?= $memo['file_path'] ?>', '<?= htmlspecialchars($memo['description']) ?>', '<?= date('F j, Y', strtotime($memo['posted_on'])) ?>', '<?= basename($memo['file_path']) ?>')"
                                ...
                                <div class="p-6 bg-white" style="flex-shrink: 0;">
                                    ...
                                    <div class="flex justify-center gap-4">
                                        <button class="bg-blue-500 text-white p-2 sm:p-2 rounded-full hover:bg-blue-600 transition duration-200 transform hover:scale-110" onclick="changeSlide(-1)">
        ...
        <?php else: ?>
            <div class="flex-1 flex items-center justify-center">
                <div class="no-memos-message flex flex-col justify-center items-center h-full p-4 text-center">
                    ...
                    <p class="text-gray-600 text-lg font-bold">No approved memos available</p>
                </div>
            </div>
        <?php endif; ?>
    </div>
