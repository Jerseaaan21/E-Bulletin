...
<head>
    ...
    <!-- Load Chart.js in the main dashboard -->
...
<body class="bg-gray-50 min-h-screen font-sans">
    ...
    <script>
        ...
        function updateUIWithMainData(data) {
            ...
            // Update user info
            if (data.user) {
                document.getElementById('userName').textContent = data.user.name || 'Unknown User';
                document.getElementById('userEmail').textContent = data.user.email || 'No email provided';
            }
            ...
            const createModuleItem = (module, folderName, prefix) => {
                const uniqueId = `upload-${prefix}-module-${module.id}`; // e.g., upload-main-module-1
                ...
                btn.innerHTML = `<i class="${module.icon || 'fas fa-file'} mr-3 w-5"></i> <span>${module.name}</span>`;
            ...
            // 1. POPULATE MANAGE DEPARTMENTS (ceit_modules) -> Prefix: 'ceit'
            ...
            if (data.ceit_modules && Array.isArray(data.ceit_modules)) {
                data.ceit_modules.forEach(module => {
                    const {
                        btn,
                        content
                    } = createModuleItem(module, 'CEIT_Modules', 'ceit');
                    deptContainer.appendChild(btn);
                    dynamicModuleContent.appendChild(content);
                });
            }

            // 2. POPULATE POSTING (modules) -> Prefix: 'main'
            ...
            if (data.modules && Array.isArray(data.modules)) {
                data.modules.forEach(module => {
                    const {
                        btn,
                        content
                    } = createModuleItem(module, 'Main_Modules', 'main');
        ...
        function cleanupTabResources(tabId) {
            // Check if tabId starts with either prefix
            if (tabId.startsWith('upload-main-module-') || tabId.startsWith('upload-ceit-module-')) {
        ...
        function reinitializeModule(tabId) {
            if (tabId.startsWith('upload-main-module-') || tabId.startsWith('upload-ceit-module-')) {
                ...
                let module = mainData?.modules?.find(m => m.id == realId) || mainData?.ceit_modules?.find(m => m.id == realId);

                if (module) {
                    const lowerName = module.name.toLowerCase();
                    if (lowerName.includes('announcement') && typeof initializeAnnouncementsModule === 'function') initializeAnnouncementsModule();
                    else if (lowerName.includes('memo') && typeof initializeMemosModule === 'function') initializeMemosModule();
                    else if (lowerName === 'graphs' && typeof initializeCharts === 'function') setTimeout(initializeCharts, 100);
                }
            }
        }
        ...
        document.addEventListener('DOMContentLoaded', async function() {
            ...
            document.addEventListener('click', function(e) {
                ...
                if (activeTab && activeTab !== tabId) cleanupTabResources(activeTab);
                activeTab = tabId;
                ...
                if (tabId === 'upload-archive') {
                    pageTitle.textContent = 'Archive';
                } else if (tabId.startsWith('upload-')) {
                    ...
                    if (tabContent && !tabContent.hasAttribute('data-loaded')) {
        ...
        window.reloadModule = function() {
            const activeTab = document.querySelector('.tab-content.active');
            if (activeTab && activeTab.id.startsWith('upload-')) {
                const moduleName = activeTab.getAttribute('data-module-name');
                const folderName = activeTab.getAttribute('data-folder');
                if (moduleName && folderName) {
                    loadModuleContent(activeTab.id, moduleName, folderName);
                }
            }
        }
        window.closeFileModal = function(status, index) {
        ...
        window.closeMemoDeleteModal = function() {
