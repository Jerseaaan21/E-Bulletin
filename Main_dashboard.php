...
// Get user ID from session
 $userId = null;
 $deptAcronym = 'MIS'; // Default value
if (isset($_SESSION['user_info'])) {
    // Get user info from session
    $userEmail = $_SESSION['user_info']['email'];

    // Fetch user data from database
    $query = "SELECT u.*, d.acronym FROM users u LEFT JOIN departments d ON u.dept_id = d.dept_id WHERE u.email = ?";
    ...
    if ($user) {
        $userId = $user['id'];
        $deptAcronym = $user['acronym'] ? $user['acronym'] : 'MIS';
    }
}
...
<head>
    ...
    <title><?php echo htmlspecialchars($deptAcronym); ?> Dashboard</title>
    ...
    <style>
        ...
        .sidebar-item.no-animation:hover {
            transform: none;
        }
...
<body class="bg-gray-50 min-h-screen font-sans">
    ...
    <div class="flex h-screen overflow-hidden">
        <!-- Sidebar -->
        <aside id="sidebar" class="sidebar w-80 bg-gradient-to-b from-orange-500 to-orange-600 text-white flex flex-col shadow-xl">
            <!-- Fixed Header -->
            <div class="flex-shrink-0 p-4">
                <div class="flex flex-col items-center">
                    <img src="cvsulogo.png" alt="School Logo" class="w-16 h-16 object-contain drop-shadow-[0_5px_10px_rgba(0,0,0,0.3)]" />
                    <h2 class="mt-1 text-lg font-bold"><?php echo htmlspecialchars($deptAcronym); ?> Dashboard</h2>
                </div>
            </div>

            <!-- Scrollable Navigation -->
            <div class="flex-1 overflow-y-auto px-2" style="scrollbar-width: thin; scrollbar-color: rgba(255,255,255,0.3) transparent;">
                <nav class="space-y-1 pb-4">
                    <!-- SECTION 1: Manage Departments (Accordion) -->
                    <div class="mb-1">
                        <div class="accordion-header active sidebar-section flex items-center justify-between cursor-pointer" onclick="toggleAccordion('manage-dept')">
                            <span>Manage Departments</span>
                            <i id="icon-manage-dept" class="fas fa-chevron-down accordion-icon text-sm rotate"></i>
                        </div>
                        <!-- Modules from Manage_Modules folder go here -->
                        <div id="list-manage-dept" class="accordion-content active"></div>
                    </div>

                    <!-- SECTION 2: Posting (Accordion) -->
                    <div class="mb-1">
                        <div class="accordion-header sidebar-section flex items-center justify-between cursor-pointer" onclick="toggleAccordion('posting')">
                            <span>Posting</span>
                            <i id="icon-posting" class="fas fa-chevron-down accordion-icon text-sm"></i>
                        </div>
                        <!-- Modules from CEIT_Modules folder go here -->
                        <div id="list-posting" class="accordion-content"></div>
                    </div>
                </nav>
            </div>
            <div class="border-t border-orange-400/30" style="box-shadow: 0 -2px 8px rgba(0, 0, 0, 0.15);">
                <a href="Bulletin.php?dept_id=<?php echo htmlspecialchars($user['dept_id']); ?>" target="_blank" class="sidebar-item no-animation w-full text-left p-3 text-sm hover:bg-orange-700 flex items-center text-white">
                    <i class="fas fa-tv mr-3 w-5"></i> <span>View Bulletin</span>
                </a>
                <button data-tab="upload-archive" class="upload-tab-btn sidebar-item no-animation w-full text-left p-3 text-sm hover:bg-orange-700 flex items-center">
            ...
            <div class="flex-shrink-0 p-4 border-t border-orange-400">
                <div class="p-3 bg-orange-700 rounded-lg">
    ...
    <script>
        ...
        function toggleAccordion(sectionId) {
            const content = document.getElementById(`list-${sectionId}`);
            const icon = document.getElementById(`icon-${sectionId}`);
            const header = icon ? icon.closest('.accordion-header') : null;
            
            // Get all accordion sections
            const allSections = ['manage-dept', 'posting'];
            
            // Close all other sections
            allSections.forEach(section => {
                if (section !== sectionId) {
                    const otherContent = document.getElementById(`list-${section}`);
                    const otherIcon = document.getElementById(`icon-${section}`);
                    const otherHeader = otherIcon ? otherIcon.closest('.accordion-header') : null;
                    if (otherContent && otherIcon && otherHeader) {
        ...
        function showNotification(message, type = 'success') {
            ...
            notification.className = `fixed bottom-4 right-4 z-50 bg-${bgColor}-500 text-white px-4 py-3 rounded-lg shadow-lg mb-2`;
